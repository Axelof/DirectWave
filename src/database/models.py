...
from regexps import CHAT_LINK_REGEX, EMAIL_REGEX
...
class User(BaseModel, CreatedAtMixin):
    ...
    email = fields.TextField(max_length=256, regex=EMAIL_REGEX, index=True, unique=True)
...
class Chat(BaseModel, CreatedAtMixin):
    ...
    link = fields.TextField(
        min_length=3, max_length=6, regex=CHAT_LINK_REGEX, index=True, unique=True
    )
